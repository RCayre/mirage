...
class BtHCIDevice(wireless.Device):
    ...
    def _initBT(self):
        ...
        self._internalCommand(
            HCI_Cmd_Set_Event_Mask(mask=b"\xFF\xFF\xFB\xFF\x07\xF8\xBF\x3D")
        )
    ...
    def init(self):
        ...
        # Custom flag for zephyr MITM mode
        self.mitm = False
    ...
    def recv(self):
        ...
        try:
            if (
                    ...
                    and self.socket.readable()
            ):
                packet = self._recv()
                # If mitm mode with nRF zephyr controller is used.
                if (packet.type == 0x02 and self.mitm):
                    return packet
                # Packet reassembly for ACL Data Packets
                # Check if ACL Data Packet
                # and (PB == 0x02 or PB == 0x00) automatical and non-automatical flushable packet
                # Since 0x1b is max length of ACL Data packet
                # continuing fragment is possible
                elif (
                        packet.type == 0x02
                        # and packet.len == 0x1B
                        and (packet.PB == 0x02 or packet.PB == 0x00)
                        and packet.haslayer(L2CAP_Hdr)
                ):
                    tmpL2CAPHdr = packet.getlayer(L2CAP_Hdr)
                    # Check length of payload, if already complete
                    if tmpL2CAPHdr.len == len(tmpL2CAPHdr.payload):
                        self._exitListening()
                        return packet
                    # Save first fragment for reassembly
                    else:
                        self.l2CapLen = tmpL2CAPHdr.len
                        self.l2CapCid = tmpL2CAPHdr.cid
                        self.l2CapPayload = raw(tmpL2CAPHdr.payload)
                        return None
                # Check if ACL Data packet with PB flag set to 0x01,
                # if true packet is continuing fragment
                elif (
                        packet.type == 0x02
                        and packet.PB == 0x01
                        and self.l2CapLen != -1
                    ...
                    self.l2CapPayload += raw(packet.getlayer(HCI_ACL_Hdr).payload)

                    # Check length of payload, if already complete
                    if self.l2CapLen == len(self.l2CapPayload):
                        self._exitListening()
                        tmpPkt = (
                            HCI_Hdr()
                            / HCI_ACL_Hdr()
                            # Remove one for the opcode byte
                            / L2CAP_Hdr(len=self.l2CapLen - 1, cid=self.l2CapCid)
                        )
                        if self.l2CapCid == 0x06:
                            tmpPkt /= SM_Hdr(self.l2CapPayload)
                        elif self.l2CapCid == 0x04:
                            tmpPkt /= ATT_Hdr(self.l2CapPayload)
                        else:
                            tmpPkt /= L2CAP_CmdHdr(self.l2CapPayload)
                        self.l2CapLen = -1
                        self.l2CapCid = -1
                        self.l2CapPayload = ""
                        return tmpPkt
    ...
    def _internalCommand(self, cmd, noResponse=False):
        ...
        if not noResponse:
            ...
            if (
                    response.type == 0x04
                    and response.code == 0xE
                    and response.opcode == cmd.opcode
            ):
                if response.status != 0:
                    raise BluetoothCommandError(
                        "Command %x failed with %x" % (
                            cmd.opcode, response.status)
                    )
                return response
    ...
    def _setCurrentHandle(self, handle, address=""):
        if handle != -1:
            ...
            if not found:
                self.handles.append(
                    {
                        "address": address.upper() if address is not None else "",
                        "handle": handle,
                    }
                )
    ...
    def enableMitM(self, flag):
        """
        This method sets the mitm flag to the requried value. 
        This method is only requried for nrf zephyr controller mitm mode to forward encrypted packets.

        :param flag: boolean indicating if mitm is activated
        :type flag: bool
    ...
    def setAddress(self, address):
        ...
        if self.isAddressChangeable():
            self._enterCommandMode()
            io.info(
                "Changing HCI Device (" + self.interface +
                ") Address to : " + address
            )
            response = self._internalCommand(
                HCI_Cmd_Read_Local_Version_Information())
            ...
            if manufacturer not in COMPATIBLE_VENDORS:
                ...
                self._internalCommand(
                    HCI_Cmd_CSR_Write_BD_Address(addr=address), noResponse=True
                )
...
class BluetoothEmitter(wireless.Emitter):
    ...
    def __init__(self, interface="hci0"):
        super().__init__(
            interface=interface, packetType=BluetoothPacket, deviceType=BtHCIDevice
        )
    ...
    def convert(self, p):
        if isinstance(p, BluetoothConnect):
            ...
            p.packet = (
                ...
                / HCI_Cmd_Inquiry(
                    lap=p.lap,
                    num_response=p.numResponses,
                    inquiry_length=p.inquiryLength,
                )
            )
            ...
            p.packet = (
                ...
                / HCI_Cmd_Remote_Name_Request(
                    addr=p.address, page_scan_repetition_mode=p.pageScanRepetitionMode
                )
            )
            ...
            p.packet = (
                ...
                / HCI_Cmd_Accept_Connection_Request(
                    addr=p.address, role_switch=0x01 if p.role == "slave" else 0x00
                )
            )
            ...
            p.packet = (
                HCI_Hdr()
                / HCI_Command_Hdr()
                / HCI_Cmd_Reject_Connection_Request(addr=p.address, reason=p.reason)
            )
        elif isinstance(p, BluetoothL2CAPConnectionRequest):
            handle = (
                p.connectionHandle
                if p.connectionHandle != -1
                else self.device.getCurrentHandle()
            )
            p.packet = (
                HCI_Hdr()
                / HCI_ACL_Hdr(handle=handle)
                / L2CAP_Hdr(cid=1)
                / L2CAP_CmdHdr()
                / L2CAP_ConnReq(psm=p.psm, scid=p.scid)
            )  # cid == 1 : L2CAP signaling channel
            ...
            p.packet = (
                ...
                / L2CAP_CmdHdr()
                / L2CAP_ConnResp(
                    dcid=p.dcid, scid=p.scid, status=p.status, result=p.result
                )
            )  # cid == 1 : L2CAP signaling channel
            ...
            p.packet = (
                ...
                / L2CAP_Hdr(cid=1)
                / L2CAP_CmdHdr()
                / L2CAP_InfoReq(type=p.type, data=p.data)
            )  # cid == 1 : L2CAP signaling channel
            ...
            p.packet = (
                ...
                / L2CAP_Hdr(cid=1)
                / L2CAP_CmdHdr()
                / L2CAP_InfoResp(type=p.type, result=p.result, data=p.data)
            )  # cid == 1 : L2CAP signaling channel
    ...
    def convert(self, packet):
        ...
        if "hci" in self.interface:
            if HCI_Evt_Extended_Inquiry_Result in packet:
                return BluetoothInquiryScanResult(
                    address=packet.addr,
                    numResponses=packet.num_response,
                    classOfDevice=packet.class_of_device,
                    rssi=packet.rssi,
                    data=packet.data,
                )
                ...
                if packet.status == 0x00:
                    ...
                    io.info(
                        "Uploading connection handle : "
                        + str(self.device.getCurrentHandle())
                    )
                return BluetoothConnectResponse(
                    srcMac=packet.addr,
                    success=(packet.status == 0x00),
                    encryptionMode=(packet.encryption_mode == 0x01),
                ...
                return BluetoothRemoteNameResponse(
                    success=(packet.status == 0x00),
                    addr=packet.addr,
                    remoteName=packet.remote_name,
                )
            elif HCI_Evt_Connection_Request in packet:
                return BluetoothConnectionRequest(
                    addr=packet.addr, classOfDevice=packet.class_of_device
                )
            elif L2CAP_ConnReq in packet:
                return BluetoothL2CAPConnectionRequest(
                    scid=packet.scid, psm=packet.psm, connectionHandle=packet.handle
                )
            elif L2CAP_ConnResp in packet:
                return BluetoothL2CAPConnectionResponse(
                    scid=packet.scid,
                    dcid=packet.dcid,
                ...
                return BluetoothL2CAPInformationRequest(
                    type=packet.type, data=packet.data, connectionHandle=packet.handle
                )
            elif L2CAP_InfoResp in packet:
                return BluetoothL2CAPInformationResponse(
                    type=packet.type,
                    data=packet.data,
                    result=packet.result,
                    connectionHandle=packet.handle,
                ...
                return BluetoothL2CAPConfigurationRequest(
                    dcid=packet.dcid, flags=packet.flags, connectionHandle=packet.handle
                )
            elif L2CAP_ConfResp in packet:
                return BluetoothL2CAPConfigurationResponse(
                    scid=packet.scid,
                    flags=packets.flags,
                    result=packet.result,
                    connectionHandle=packet.handle,
