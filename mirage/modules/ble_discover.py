...
class ble_discover(module.WirelessModule):
    ...
    def exportAttributes(self, attributes):
        ...
        with open(self.args["ATT_FILE"], "w") as outfile:
            ...
            io.success(
                "Discovered attributes are saved as "
                + self.args["ATT_FILE"]
                + " (CFG file format)"
            )
    ...
    def exportGATT(self, datas):
        ...
        for element in datas:
            ...
            for characteristic in element["characteristics"]:
                ...
                security = (
                    ...
                    if characteristic["security"] == "None"
                    else characteristic["security"].hex()
                )
        ...
        with open(self.args["GATT_FILE"], "w") as outfile:
            config.write(outfile)
            io.success(
                "Discovered services and characteristics are saved as "
                + self.args["GATT_FILE"]
                + " (CFG file format)"
            )
    ...
    def serviceToString(self, service):
        uuid128 = (
            service["uuid"].UUID128.hex() if service["uuid"].UUID128 is not None else ""
        )
        ...
        handles = (
            "(start Handle = "
            + "0x{:04x}".format(service["startHandle"])
            + " / end Handle = "
            + "0x{:04x}".format(service["endHandle"])
            + ")"
        )
    ...
    def printCharacteristics(self, characteristics, title="Characteristics"):
        ...
        for characteristic in characteristics:
            ...
            uuid16 = (
                hex(characteristic["uuid"].UUID16)
                if characteristic["uuid"].UUID16 is not None
                else ""
            )
            uuid128 = (
                characteristic["uuid"].UUID128.hex()
            ...
            value = (
                characteristic["value"].replace(b"\x00", b"").decode("ascii")
            ...
            if "descriptors" in characteristic:
                for desc in characteristic["descriptors"]:
                    namedesc = (
                        ble.CharacteristicDescriptor(data=desc["type"]).UUID.name
                        if ble.CharacteristicDescriptor(data=desc["type"]).UUID.name
                        != None
                        else ""
                    )
                    valuedesc = (
    ...
    def printServices(self, services, title="Services"):
        ...
        for service in services:
            ...
            endHandle = "0x{:04x}".format(service["endHandle"])
            uuid16 = (
                hex(service["uuid"].UUID16)
                if service["uuid"].UUID16 is not None
                else ""
            )
            uuid128 = (
                service["uuid"].UUID128.hex()
    ...
    def printAttributes(self, attributes):
        ...
        for attribute in attributes:
            ...
            attributeValue = (
                attribute["value"].replace(b"\x00", b"").decode("ascii")
                if utils.isPrintable(attribute["value"])
                else attribute["value"].hex()
            )
    ...
    def servicesDiscovery(self, uuid, startHandle=0x0001, endHandle=0xFFFF):
        ...
        while continuer:
            ...
            request = ble.BLEReadByGroupTypeRequest(
                startHandle=start, endHandle=end, uuid=uuid
            )

            self.emitter.sendp(request)

            p = self.receive(
                [ble.BLEReadByGroupTypeResponse, ble.BLEErrorResponse], retry=request
            )
    ...
    def primaryServicesDiscovery(self, startHandle=0x0001, endHandle=0xFFFF):
        ...
        return self.servicesDiscovery(
            uuid, startHandle=startHandle, endHandle=endHandle
        )
    ...
    def characteristicsDiscovery(self, startHandle=0x0001, endHandle=0xFFFF):
        ...
        while continuer:
            request = ble.BLEReadByTypeRequest(
                startHandle=start, endHandle=end, uuid=characteristicDeclarationUUID
            )
            self.emitter.sendp(request)

            p = self.receive(
                [ble.BLEReadByTypeResponse, ble.BLEErrorResponse], retry=request
            )
            ...
            if isinstance(p, ble.BLEReadByTypeResponse):
                ...
                for i in p.attributes:
                    ...
                    if "Read" in characteristicDeclaration.permissionsFlag:
                        ...
                        self.emitter.sendp(request)
                        p = self.receive(
                            [ble.BLEReadResponse, ble.BLEErrorResponse], retry=request
                        )
                        if isinstance(p, ble.BLEReadResponse):
                            characteristic["value"] = p.value
                            characteristic["security"] = "None"
                        else:
                            characteristic["security"] = bytes([p.ecode])
    ...
    def attributesDiscovery(self, startHandle=0x0001, endHandle=0xFFFF):
        ...
        while continuer:
            ...
            self.emitter.sendp(request)
            p = self.receive(
                [ble.BLEFindInformationResponse, ble.BLEErrorResponse], retry=request
            )
            ...
            if isinstance(p, ble.BLEFindInformationResponse):
                for i in p.attributes:
                    ...
                    if attribute not in descriptors:
                        ...
                        p2 = self.receive(
                            [ble.BLEReadResponse, ble.BLEErrorResponse], retry=request2
                        )
    ...
    def characteristicsByServiceDiscovery(self, service):
        return self.characteristicsDiscovery(
            startHandle=service["startHandle"], endHandle=service["endHandle"]
        )
    ...
    def run(self):
        ...
        if self.checkCapabilities():
            if self.receiver.isConnected():
                for what in utils.listArg(self.args["WHAT"]):
                    ...
                    if what == "primaryservices":
                        ...
                        for service in services:
                            characteristics = self.characteristicsByServiceDiscovery(
                                service
                            )
                            ...
                            for i in range(0, len(characteristics) - 1):
                                ...
                                if end - start >= 0:
                                    characteristics[i][
                                        "descriptors"
                                    ] = self.characteristicDescriptorDiscovery(
                                        start, end
                                    )
                            ...
                            alldatas.append(
                                {"service": service, "characteristics": characteristics}
                            )
                            self.printCharacteristics(
                                characteristics, self.serviceToString(service)
                            )
            ...
            io.fail(
                ...
                + ") is not able to communicate as master."
            )
            return self.nok()
